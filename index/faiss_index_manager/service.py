...
class FaissWrapper():
    def __init__(self, index, ids) -> None:
        ...
        self.id_map = {_id: i for i, _id in enumerate(ids)}  # Cho internal search
...
def load_index(feature_type):
    ...
    index_wrapper = None
    index_path = f'./collection_dir/faiss-index_{feature_type}.faiss'
    idmap_path = f'./collection_dir/faiss-idmap_{feature_type}.txt'
...
def search():
    ...
    if index is None:
        return f"No index found for '{feature_type}' features.", 400  # BAD_REQUEST?c0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--port', default='8082', help="Port to use for binding")
    ...
    if not args.lazy_load:
        config = load_config('/collection_dir/config.yaml')
    ...
    # python -m services.index.faiss_index_manager.service --lazy-load